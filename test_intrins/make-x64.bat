...
@rem SSE4-only native x64 build
cl -FAsc -Zi -O2 -I../dvec_demo -I.. -arch:SSE4.2 -Tc test-intrins.c -link -out:test-intrins-sse4.exe -debug -release -incremental:no

@rem SSE4+AVX2 native x64 build
cl -FAsc -Zi -O2 -I../dvec_demo -I.. -arch:AVX2   -Tc test-intrins.c -link -out:test-intrins-avx2.exe -debug -release -incremental:no
...
if exist test-intrins-sse4.exe (%LOADER% test-intrins-sse4.exe    -o test-sse4.txt)
if exist test-intrins-avx2.exe (%LOADER% test-intrins-avx2.exe    -o test-avx2.txt)

if exist test-intrins-sse4.exe (%LOADER% test-intrins-sse4.exe -b -o bench-sse4.txt)
if exist test-intrins-avx2.exe (%LOADER% test-intrins-avx2.exe -b -o bench-avx2.txt)
